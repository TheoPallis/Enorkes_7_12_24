...
from Helper_Scripts.Preprocessing.Formatting.formatting import (
    ...
    remove_invalid_fill_na_convert_time,
    filter_df,
    fill_lawyers,
...
from Helper_Scripts.Extract_Text.Extact_Text import get_text_and_date
...
from Helper_Scripts.Create_Excel_File.Create_Excel_File import create_excel_file
from Config.Config import out_path, mapping_dikigoron_excel_file, dedie_path

def main_function(excel_file_path,filtered_date=None,filtered_name_list=None,head=None) :
    anathesi_df = load_excel_file(excel_file_path)
    anathesi_df = remove_invalid_fill_na_convert_time(anathesi_df)
    anathesi_df = filter_df(anathesi_df,filtered_date,filtered_name_list,head)
...
def test_main_function() :
    ...
    output_excel_file_path = r"C:\Users\pallist\Desktop\Desktop\ΤΡΕΧΟΝΤΑ\Ένορκες\90 12_12 testing.xlsx"
    _,test_kliseis_df = main_function(test_excel_file_path)
    kliseis_df = pd.read_excel(output_excel_file_path,sheet_name='Κλήσεις')
    assert_frame_equal(kliseis_df, test_kliseis_df)
